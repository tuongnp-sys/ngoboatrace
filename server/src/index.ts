...
import { loadDb, saveDb, todayKey, type DbPlayer, type DbScore } from './store.js';
...
app.post<{ Body: { deviceId: string; displayName?: string } }>('/auth/guest', async (req, reply) => {
  const { deviceId, displayName = 'Người chơi' } = req.body ?? {};
  if (!deviceId) {
...
app.put<{ Body: { profile: Record<string, unknown> } }>('/player/me', async (req, reply) => {
  ...
  if (typeof req.body.profile?.displayName === 'string') {
    db.players[idx]!.displayName = req.body.profile.displayName;
  }
  saveDb(db);
...
app.post<{ Body: SubmitBody }>('/scores', async (req, reply) => {
  ...
  const entry: DbScore = {
    ...
    playerId: player.id,
    displayName: body.displayName ?? player.displayName,
    mode: 'daily',
    seed: body.seed,
